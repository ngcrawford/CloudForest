...
class BootstrapAWS(MRJob):
    ...
    #DEFAULT_OUTPUT_PROTOCOL = 'raw_value' # prevent quotes in output
    ...
    def bootstrapReplicates(self, key, line):
        ...
        Rather that producing all the bootstrap replicates it take a file of duplicated
        datasets equal to the number of replicates. One boostrap replicate is produced
        from the loci within each dataset, and one bootstap replicate is made on each locus
        of the bootstrapped loci.  This speed up the MapReduce algorithm because by
        parallelizing the bootstrapping opperation.
        
        This fuction also keeps the appropriate model of molecular evolution with each locus.
        """
    ...
    def phyml(self, key, line):
        ...
        if len(line.split("\t")) == 2:
            key, line = line.split("\t")
        # line = line.strip("\"")             # weirdness parsing key, value
        # key = int(key)
        ...
        # RUN PHYML
        cli = '%s/%s/./%s --input=%s --model=%s >/dev/null 2>&1' % (os.getcwd(),'bin', phyml_exe, temp_in.name, model) 
        cli_parts = cli.split()
